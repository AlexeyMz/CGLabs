...
const char *sphereVertex = "\
	uniform mat4 MVP;\
	layout(location = 0) in vec3 Vertex;\
    out vec3 position;\
    void main() {\
      position = sphereCenter + Vertex.xyz * sphereRadius;\
      gl_Position = MVP * vec4(position, 1.0);\
    }\
  ";

const char *sphereFragment = "\
    in vec3 position;\
	layout(location = 0) out vec4 FragColor;\
    void main() {\
